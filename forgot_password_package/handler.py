...
Generates a reset code, stores it in DynamoDB, sends via Resend.
"""
...
if '@gmail.com' in SENDER or '@' not in SENDER:
    ...
    print(f"[DEBUG] Using verified domain: {SENDER}")
...
def send_reset_email(email: str, code: str):
    print(f"[DEBUG] send_reset_email called for {email}")
    print(f"[DEBUG] RESEND_API_KEY present: {bool(RESEND_API_KEY)}")
    print(f"[DEBUG] SENDER: {SENDER}")
    
    if not RESEND_API_KEY:
        print(f"[EMAIL SKIP] RESEND_API_KEY not set. Reset code for {email}: {code}")
        log_info(f"[EMAIL SKIP] RESEND_API_KEY not set. Reset code for {email}: {code}")
        return
    ...
    text = f"Your MaatriSahayak password reset code is: {code}\n\nThis code expires in 15 minutes.\n\nIf you did not request this, ignore this email."

    try:
        payload = json.dumps({
            'from': f'MaatriSahayak <{SENDER}>',
            'to': [email],
            'subject': 'MaatriSahayak - Your Password Reset Code',
            'html': html,
            'text': text,
        }).encode('utf-8')
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                'Content-Type': 'application/json',
                'User-Agent': 'MaatriSahayak/1.0',
            },
            method='POST'
        )
        with urllib.request.urlopen(req, timeout=15) as resp:
            response_body = resp.read().decode('utf-8')
            print(f"[EMAIL OK] Reset code sent to {email} | status={resp.status} | response={response_body}")
            log_info(f"[EMAIL OK] Reset code sent to {email} | status={resp.status} | response={response_body}")
    except urllib.error.HTTPError as e:
        error_body = e.read().decode('utf-8')
        print(f"[EMAIL ERROR] HTTP {e.code}: {error_body} | to={email}")
        log_error(f"[EMAIL ERROR] HTTP {e.code}: {error_body} | to={email}", Exception(e))
        # Log the code for debugging if email fails
        print(f"[EMAIL FAILED - CODE FOR DEBUG] {email}: {code}")
        log_info(f"[EMAIL FAILED - CODE FOR DEBUG] {email}: {code}")
    except Exception as e:
        print(f"[EMAIL ERROR] {str(e)} | to={email}")
