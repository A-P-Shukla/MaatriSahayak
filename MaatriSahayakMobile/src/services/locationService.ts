...
const villageCache: { [district: string]: string[] } = {};
const districtCache: string[] = [];
 ...
 */
export const fetchVillagesByDistrict = async (district: string): Promise<string[]> => {
    // Return cached data if available
    if (villageCache[district]) {
        return villageCache[district];
    }

    try {
        // Option 1: Using India Post API (free, no auth required)
        // This API provides post office names which often correspond to villages
        const response = await axios.get(
            `https://api.postalpincode.in/postoffice/${encodeURIComponent(district)}`
        );

        if (response.data && response.data[0]?.Status === 'Success') {
            const villages = response.data[0].PostOffice.map((po: any) => po.Name)
            ...
            // Cache the result
            villageCache[district] = uniqueVillages;
            return uniqueVillages;
        }

        // Fallback to common village names if API fails
        return getCommonVillageNames();
    } catch (error) {
        console.error('Error fetching villages:', error);
        // Return common village names as fallback
        return getCommonVillageNames();
    }
};

/**
 * Search villages by partial name within a district
 * @param district - District name
 * @param searchTerm - Partial village name
 * @returns Filtered array of village names
 */
export const searchVillages = async (district: string, searchTerm: string): Promise<string[]> => {
    const villages = await fetchVillagesByDistrict(district);
    
    if (!searchTerm || searchTerm.trim() === '') {
        return villages.slice(0, 50); // Return first 50 villages
    }

    const term = searchTerm.toLowerCase();
    return villages
        .filter(village => village.toLowerCase().startsWith(term))
        .slice(0, 50); // Limit to 50 results
};

/**
 * Get all districts in India
...
export const getAllDistricts = (): string[] => {
    return [
        ...
        // Punjab
        'Amritsar', 'Barnala', 'Bathinda', 'Faridkot', 'Fatehgarh Sahib', 'Fazilka', 'Ferozepur', 'Gurdaspur', 'Hoshiarpur', 'Jalandhar', 'Kapurthala', 'Ludhiana', 'Mansa', 'Moga', 'Muktsar', 'Pathankot', 'Patiala', 'Rupnagar', 'Sangrur', 'SAS Nagar', 'Shaheed Bhagat Singh Nagar', 'Tarn Taran',
        // Rajasthan
        ...
        // Uttar Pradesh
        'Agra', 'Aligarh', 'Allahabad', 'Ambedkar Nagar', 'Amethi', 'Amroha', 'Auraiya', 'Azamgarh', 'Baghpat', 'Bahraich', 'Ballia', 'Balrampur', 'Banda', 'Barabanki', 'Bareilly', 'Basti', 'Bhadohi', 'Bijnor', 'Budaun', 'Bulandshahr', 'Chandauli', 'Chitrakoot', 'Deoria', 'Etah', 'Etawah', 'Ayodhya', 'Farrukhabad', 'Fatehpur', 'Firozabad', 'Gautam Buddha Nagar', 'Ghaziabad', 'Ghazipur', 'Gonda', 'Gorakhpur', 'Hamirpur', 'Hapur', 'Hardoi', 'Hathras', 'Jalaun', 'Jaunpur', 'Jhansi', 'Kannauj', 'Kanpur Dehat', 'Kanpur Nagar', 'Kasganj', 'Kaushambi', 'Kushinagar', 'Lakhimpur Kheri', 'Lalitpur', 'Lucknow', 'Maharajganj', 'Mahoba', 'Mainpuri', 'Mathura', 'Mau', 'Meerut', 'Mirzapur', 'Moradabad', 'Muzaffarnagar', 'Pilibhit', 'Pratapgarh', 'Prayagraj', 'Raebareli', 'Rampur', 'Saharanpur', 'Sambhal', 'Sant Kabir Nagar', 'Shahjahanpur', 'Shamli', 'Shravasti', 'Siddharthnagar', 'Sitapur', 'Sonbhadra', 'Sultanpur', 'Unnao', 'Varanasi',
        // Uttarakhand
        ...
        // Union Territories
        'Andaman and Nicobar Islands', 'Chandigarh', 'Dadra and Nagar Haveli and Daman and Diu', 'Delhi', 'Jammu', 'Srinagar', 'Leh', 'Kargil', 'Lakshadweep', 'Puducherry', 'Karaikal', 'Mahe', 'Yanam',
    ].sort();
};

/**
 * Fallback common village names when API fails
 */
const getCommonVillageNames = (): string[] => {
    return [
        'Rampur', 'Ramgarh', 'Rampur Kalan', 'Sitapur', 'Sultanpur', 'Bahadurpur', 'Bhagwanpur',
        'Chandpur', 'Devipur', 'Fatehpur', 'Gopalpur', 'Haripur', 'Jaipur', 'Kamalpur', 'Laxmipur',
        'Madhavpur', 'Nandpur', 'Rajpur', 'Shivpur', 'Vishwanathpur', 'Anandpur', 'Balrampur',
        'Chandanpur', 'Durgapur', 'Ganeshpur', 'Hanumanganj', 'Indrapur', 'Jagannathpur', 'Keshavpur',
        'Lakshmanpur', 'Maheshpur', 'Narayanpur', 'Omkareshwar', 'Parshuram', 'Raghunathpur',
        'Shankarpur', 'Trivenipur', 'Umapura', 'Vasudevpur', 'Yadavpur',
    ].sort();
};

/**
 * Clear village cache (useful when switching districts)
 */
export const clearVillageCache = (district?: string) => {
    if (district) {
        delete villageCache[district];
    } else {
        Object.keys(villageCache).forEach(key => delete villageCache[key]);
    }
};
